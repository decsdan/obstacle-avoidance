...
class DWAFollower(Node):
    ...
    def __init__(self):
        ...
        self._twist_pub = self.create_publisher(
            TwistStamped, f'{self.ns}/cmd_vel_raw', qos_cmd)
    ...
    def _declare_params(self):
        ...
        self.declare_parameter('max_velocity', 0.26)
        ...
        # FIXME: e-stop dist should come from URDF, not hardcoded
        ...
        self.declare_parameter('weights.blend_w_goal', 0.60)
        self.declare_parameter('weights.blend_w_heading', 0.25)
        self.declare_parameter('weights.blend_w_dist_factor', 0.7)
    ...
    def _read_params(self):
        ...
        self.blend_w_goal = self.get_parameter('weights.blend_w_goal').value
        self.blend_w_heading = self.get_parameter('weights.blend_w_heading').value
        self.blend_w_dist_factor = self.get_parameter('weights.blend_w_dist_factor').value
    ...
    def _execute(self, goal_handle):
        ...
        while rclpy.ok():
            ...
            # front-cone e-stop: hold until clear, or give up
            ...
            if scores[best_idx] == -np.inf:
                # every sampled trajectory hits an obstacle
    ...
    def _cost(self, trajectories, final_vs, final_ws, obstacles,
        ...
        if dist_to_goal < 0.5:
            ...
            w_goal_eff = self.w_goal + t_blend * (self.blend_w_goal - self.w_goal)
            w_velocity_eff = self.w_velocity * (1.0 - t_blend)
            w_dist_path_eff = self.w_dist_path * (1.0 - t_blend * self.blend_w_dist_factor)
            w_heading_path_eff = (
                self.w_heading_path + t_blend * (self.blend_w_heading - self.w_heading_path))
        else:
