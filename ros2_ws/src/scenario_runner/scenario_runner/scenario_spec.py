...
One dataclass per YAML block mirrors the schema in PRD \u00a74.1. The loader
is strict: missing required fields raise ``ScenarioSchemaError`` with
the offending field named, so malformed scenario YAMLs fail at batch
start rather than mid-episode.
"""
...
class LoggingConfig:
    ...
    include_mape_k_markers: bool = True
...
def load_scenario(path: str) -> Scenario:
    ...
    return Scenario(
        ...
        logging=LoggingConfig(
            ...
            include_mape_k_markers=bool(
                log_raw.get('include_mape_k_markers', True)),
