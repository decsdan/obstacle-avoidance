...
Monitors pose, sim-time, and the ``conditions/collision`` latched Bool;
emits exactly one of ``reached``, ``collision``, ``timeout``, ``stuck``,
or ``cancelled`` the moment its precondition is satisfied. Collision
wins over every other non-terminal state (PRD \u00a75.6); the scenario
runner calls ``update`` once per control cycle and aborts as soon as
``terminal`` becomes non-None.
"""
