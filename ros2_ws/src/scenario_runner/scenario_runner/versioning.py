...
Records the package commit SHA, ROS distro, Gazebo version, Python
version, and OS kernel at episode start so future replays can detect
mismatched environments (PRD \u00a74.5). Every field degrades to the string
``'unknown'`` when the probe fails; metadata capture never blocks an
