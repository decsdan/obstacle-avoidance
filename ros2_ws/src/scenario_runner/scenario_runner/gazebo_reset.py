...
Wraps the two service calls the scenario runner needs per-episode:
``/world/<world>/control`` to reset the world, and
``/world/<world>/set_pose`` to teleport the robot model to the scenario
start pose. The helpers expose coroutine-style ``call`` methods so the
runner can ``await`` them inside its episode coroutine.

Relaunch-on-drift fallback is handled at the runner level, not here;
